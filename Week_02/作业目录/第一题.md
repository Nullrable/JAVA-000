...
* 另外比较有意思的是，整个堆扩容是每次发生在Full GC 之后，第一次GC，增加88m，第二次增加86.5m，第三次67m，第四次14.5m 总共增加 88m + 86.5m + 67m + 14.5m = 256m，跟初始化的 256m 累加就是512m，跟-Xmx512m一致
* 总结：如果不设置-Xms后，JVM会初始化堆的大小为 -Xmx的一半，然后Full GC时间点会较 —Xms 等于 -Xmx 时，提前，然后Full GC后就会进行扩容，直到等于-Xmx。
...
## 4. GC分析总结
可以看出，当堆内存比较小时，都会发生频繁的GC，堆内存比较大时，几个GC表现都比较好。所以不管使用哪类GC需要结合实际业务场景，设置合适的GC收集器，堆内存，线程数等参数
